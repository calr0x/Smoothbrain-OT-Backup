...
echo "Linking container backup folder to /root"
ln -sf "$(docker inspect --format='{{.GraphDriver.Data.MergedDir}}' otnode4)/ot-node/backup" /root/
echo $STATUS
if [ $STATUS == 1 ]; then
...
docker exec otnode4 node scripts/backup.js --config=/ot-node/.origintrail_noderc --configDir=/ot-node/data --backupDirectory=/ot-node/backup  2>&1
echo $?
if [ $? == 1 ]; then
...
OUTPUT=$(/root/OT-Smoothbrain-Backup/restic backup --tag otnode4 /root/backup/.origintrail_noderc /root/backup/* 2>&1)
echo $OUTPUT
if [ $? -eq 0 ]; then
