...
ln -sf "$(docker inspect --format='{{.GraphDriver.Data.MergedDir}}' otnode2)/ot-node/backup" /root/backup
echo $STATUS
if [ $STATUS == 1 ]; then
...
OUTPUT=$(/root/OT-Smoothbrain-Backup/restic backup --tag otnode2A /root/backup/.origintrail_noderc /root/backup/* 2>&1)
echo $OUTPUT
if [ $? -eq 0 ]; then
