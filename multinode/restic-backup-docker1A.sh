...
ln -sf "$(docker inspect --format='{{.GraphDriver.Data.MergedDir}}' otnode1)/ot-node/backup" /root/backup
echo $STATUS
if [ $STATUS == 1 ]; then
...
docker exec otnode1 node scripts/backup.js --config=/ot-node/.origintrail_noderc --configDir=/ot-node/data --backupDirectory=/ot-node/backup  2>&1
echo $?
if [ $? == 1 ]; then
...
OUTPUT=$(/root/OT-Smoothbrain-Backup/restic backup --tag otnode1A /root/backup/.origintrail_noderc /root/backup/* 2>&1)
echo $OUTPUT
if [ $? -eq 0 ]; then
