...
if [ $FORGET_STATUS == 0 ]; then
  /root/restic/send.sh "Forget command SUCCEEDED"
else
  /root/restic/send.sh "Forget command FAILED${N1}$FORGET_OUTPUT"
  exit 1
fi
...
if [ $PRUNE_STATUS == 0 ]; then
  /root/restic/send.sh "Prune command SUCCEEDED"
else
  /root/restic/send.sh "Prune command FAILED${N1}$PRUNE_OUTPUT"
  exit 1
fi
...
if [ $CHECK_STATUS == 0 ]; then
  /root/restic/send.sh "Check command SUCCEEDED"
else
  /root/restic/send.sh "Check command FAILED${N1}$CHECK_OUTPUT"
  exit 1
fi
