...
OUTPUT=$(mv -v /root/Smoothbrain-OT-Backup/backup/.origintrail_noderc /root/Smoothbrain-OT-Backup/backup/ 2>&1)
echo $STATUS
if [ $STATUS == 1 ]; then
  /root/Smoothbrain-OT-Backup/data/send.sh "Moving data command FAILED${N1}$OUTPUT"
  exit 1
...
OUTPUT=$(/root/Smoothbrain-OT-Backup/restic backup /root/Smoothbrain-OT-Backup/backup/.origintral_noderc /root/Smoothbrain-OT-Backup/backup/* 2>&1)
echo $STATUS
if [ $STATUS == 0 ]; then
