...
if [$? -eq 1]; then
/root/OT-Settings/data/send.sh "OT backup command FAILED:$ {
    N1
}$OUTPUT"
exit 1
fi
...
/root/OT-Settings/data/send.sh "Backup SUCCESSFUL:$ {
    N1
}$OUTPUT"
rm -rf /root/backup/* /root/backup/.origintrail_noderc
fi
else
    /root/OT-Settings/data/send.sh "Uploading backup to S3 FAILED:$ {
    N1
}$OUTPUT"
exit 1
fi
